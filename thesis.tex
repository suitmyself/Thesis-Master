\documentclass[UTF8]{pkuthss}

\usepackage[backend = biber, style = caspervector, utf8, sorting = ecnty]{biblatex}
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{deluxetable}
\usepackage{color}
\usepackage{bm}

\setlength{\bibitemsep}{3bp}
\renewcommand*{\bibfont}{\zihao{5}\linespread{1.27}\selectfont}

\pkuthssinfo{
	cthesisname = {硕士研究生学位论文}, ethesisname = {Master Thesis},
	ctitle      = {基于近场动力学理论的弹塑性材料形变及碎裂仿真},
    etitle      = {Peridynamics-Based Deformation and Fracture Animation for Elastoplastic Solids},
	cauthor     = {陈伟},
	eauthor     = {Wei Chen},
	studentid   = {1501214372},
	date        = {二〇一八\ 年\ 六\ 月},
	school      = {信息科学技术学院},
	cmajor      = {计算机软件与理论},
    emajor      = {Computer Software and Theory},
	direction   = {计算机图形学},
	cmentor     = {李胜\ 副教授},
    ementor     = {Sheng Li},
	ckeywords   = {近场动力学，形变，碎裂，弹塑性， 仿真},
    ekeywords   = {Peridynamics, Deformation, Fracture, Elastoplasticity, Animation}
}

\addbibresource{thesis.bib}

\begin{document}

    \include{chap/newcommands}

	\frontmatter
	\pagestyle{empty}
	\maketitle
	\cleardoublepage

	\include{chap/copyright}



	\cleardoublepage
	\pagestyle{plain}
	\setcounter{page}{0}
	\pagenumbering{Roman}

	\include{chap/abstract}

	\tableofcontents

	\mainmatter
	\include{chap/chap1_preface}
	\include{chap/chap2_introduction_to_animation_and_peridynamics}
    \include{chap/chap3_pdm_deformation}
    \include{chap/chap4_pdm_fracture}
    \include{chap/chap5_conclusion_and_future_work}



	\appendix
	\printbibliography[heading = bibintoc]

	\backmatter
	\include{chap/acknowledge}
    \include{chap/achievement}
	\include{chap/originauth}

\end{document}
